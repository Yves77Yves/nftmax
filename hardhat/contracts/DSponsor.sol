...
contract DSponsor is AccessControl, ContextMixin, IDSponsor {
  ...
  constructor(
    ...
    try ERC721Contract.supportsInterface(0x80ac58cd) returns (bool) {} catch (
      bytes memory
    ) {
  ...
  function getStartPeriod() external view returns (uint) {
    return startPeriod;
  }

  function getEndPeriod() external view returns (uint) {
    return endPeriod;
  }
